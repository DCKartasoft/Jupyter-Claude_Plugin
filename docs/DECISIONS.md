...
## [2026-08-19 09:00] SageMaker deployment + per-user CloudWatch session tracking

Category: scope
Added SageMaker notebook instance support as a first-class deployment target. Two new config traits: `cloudwatch_log_group` (empty = disabled) and `cloudwatch_region` (falls back to `aws_region`). `ChatWebSocketHandler` now emits one JSON record per completed conversation turn to CloudWatch Logs — log stream per session (`{jupyter_user}/{session_uuid}`), fire-and-forget via `run_in_executor` so it never blocks the WebSocket response. The Jupyter authenticated username provides per-user attribution that Bedrock's own invocation logs cannot supply when all calls share a single SageMaker execution role ARN. Added `docs/SAGEMAKER_SETUP.md` (dedicated step-by-step guide for notebook instance setup including lifecycle configuration script) and `docs/STATUS.md` (ops reference). Version bumped to `0.1.0a2`.
Rationale: Bedrock invocation logs record the execution role ARN, not the individual user, so there is no built-in per-user cost/usage tracking on SageMaker. Application-level logging from the plugin handler solves this at zero infra overhead on the client side; the CloudFormation stacks required on the AWS side are documented in `Claude_Bedrock_CLI/Docs/SAGEMAKER-JUPYTER-INTEGRATION.md`. Lifecycle configuration script avoids manual setup steps that are wiped on instance Stop/Start; it appends to SageMaker's existing `jupyter_server_config.py` rather than overwriting it, and re-enables the default file browser which `jupyter-server-nbmodel` disables as a side effect of installation.
